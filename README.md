...
Node.js is using javascript to write software instead of only in the browser. It's single threaded, asynchronous and takes advantage of using npm modules. Express is a framework of Node.js that is light and can server SPA's, HTML files, images, audio, and can build rest web services using json.

1. Understand and explain the use of Middleware.

Middleware is software that helps add on to your program such as using HELMET middleware to protect your headers on url requests. We already use Middleware with res and req which we use to give a response back to the user and can even set up a catch all request if url fails and custom message with status code sent back to user.
...
Express routers is a method to seperate our routes with different endpoints instead of adding all routes in one file. Instead we create a file for example actions endpoint and one for projects endpoint and they can both have endpoints in those files that start with "/" instead of using "/actions". Only when we export the routers back to our main server we would use "/action" and "/projects" along with router name

1. Describe tooling used to manually test the correctness of an API.

One method to manually test an API is using Postman. Postman lets developers insert the url path for example get request and would return the data or error that is requested. It also allows developers to post or put data to the API to test if the request is successful.
